...
vector<vector<int>> threeSumBrute(vector<int>&arr){ // brute force ~ O(N^3)
    int n = arr.size();
    set<vector<int>>st;
    for(int i=0; i<n; i++){
        for(int j=i+1; j<n; j++){
...
vector<vector<int>> threeSumBetter(vector<int>&arr, int target){ // better solution - hashing ~ O(N^2)
    int n = arr.size();
    set<vector<int>>st;
    for(int i=0; i<n; i++){
        set<int>hashset;
